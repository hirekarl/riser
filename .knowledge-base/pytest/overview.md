...
- **`time_machine.travel(bare_date)` + `datetime.date.today()` is timezone-dependent, not deterministic.** A bare `datetime.date` is anchored at UTC midnight; `date.today()` then converts through the host's local timezone before returning a calendar day, so on a machine whose local timezone is behind UTC, `date.today()` resolves to the *previous* day. This surfaced as real (non-flaky-looking, but machine-dependent) failures in `apps/compliance/tests/test_services.py`'s Warning/Delinquent boundary tests. Fix/avoidance: for anything asserting an exact boundary day, pass the reference date as an explicit function argument (e.g. `calculate_status(due_date, today=...)`) instead of freezing wall-clock time — `time_machine` is fine for coarse "some date in the past/future" tests where a ±1 day shift can't change the outcome, but not for exact-boundary assertions.
