...
- **No Playwright config exists yet in this repo** — no `playwright.config.ts` or `@playwright/test` devDependency was found in `frontend/package.json`. When it's added for a Django + Vite monorepo, `webServer` will likely need to start _both_ the Vite dev server and the Django backend (either as a `webServer` array with two entries, or a wrapper script) — Playwright's `webServer` config accepts an array specifically for multi-process setups like this.
