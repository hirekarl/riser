...
**Dates:** 2026-07-26 – 2026-07-29 (compressed to 4 days — the final capstone presentation is 2026-07-29, not the 1-week/8-01 range originally assumed here) **Sprint goal:** Build the P1 polish on top of Sprint 01's working core — editing, empty states, building context, filtering, and the visual status-change moment — plus the AI narration agent and the filing-summary/proof-of-service feature (PRD Journey 3, replacing the DOB Open Data address lookup, proactively deferred to the post-capstone backlog 2026-07-22 — see `docs/sprints/day-by-day-plan.md`), and get the full set demo-ready for the final presentation (per PRD Goals/Non-Goals, `docs/prd/Riser-PRD.md` section 2c).
...
- [ ] P3/post-MVP (new, replaces the DOB Open Data address-lookup stretch) — filing-summary/proof-of-service generation (PRD Journey 3, clarified 2026-07-22).
...
- [ ] AI-narration TS type/client method (Sun 7/26); filing-summary TS type/client method + fresh-eyes contract review (Mon 7/27).
- [ ] End-to-end contract verification + best-effort cut call on filing-summary feature (Tue 7/28).
...
- [ ] AI-narration panel component, on-demand + loading state (Sun 7/26); close AI-panel edge cases + "Generate filing summary" action shell (Mon 7/27); wire filing-summary button to endpoint + download/hand-off flow (Tue 7/28).
...
- [ ] Sketch filing-summary PDF data shape/layout — research/sketch only (Fri 7/24). (DOB Open Data geocoding research deferred with the rest of the address-lookup work — no longer time-sensitive this sprint.)
- [ ] Narration-briefing service, single-turn Claude API call, mocked-client tests (Sun 7/26).
- [ ] Finish AI-agent tests/coverage; start filing-summary PDF service + `proof_of_service` model field (Mon 7/27).
- [ ] Finish filing-summary service: PDF generation edge cases, tests (Tue 7/28).
...
- [ ] AI-panel accessibility/visual QA; design filing-summary empty/success states + ledger-row "sent" indicator (Mon 7/27, after Andres pushes).
- [ ] Polish filing-summary flow + ledger-row indicator (Tue 7/28, after Andres pushes).
