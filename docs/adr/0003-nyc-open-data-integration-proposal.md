...
- ~~Who owns provisioning a Socrata app token for this environment, and does it belong in `.env.example` alongside existing config?~~ **Resolved 2026-07-24, Karl Johnson:** no token needed — anonymous access is sufficient, confirmed reachable with no rate-limit issues at demo scale (used as-is in Cornell's DOB resolver POC, `backend/apps/compliance/dob.py`). Revisit only if real usage patterns change post-MVP. See `docs/prd/Riser-PRD.md`'s own Open Questions for the matching resolution.
