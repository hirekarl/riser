...
- ⚪ **[P2]** Riser generates a plain-language narration of each device's recent state transitions (e.g., "CAT 1 due in 45 days" → "CAT 1 filed" → "Defect found, correction deadline approaching"), computed on read from ledger history — not a continuously-running background process. This is a candidate surface for the embedded-AI-agent capstone requirement under discussion in issue #1: the rule-based version ships first, and generating the narration text via an LLM is a natural upgrade path, not a new feature.
...
- Should the Journey 2 narration feed be upgraded to LLM-generated text to satisfy the capstone's embedded-AI-agent requirement? Owner: whole team — directly tied to the still-open decision in issue #1.
