...
- ~~Should an NYC Open Data app token be provisioned for higher rate limits, or is anonymous access sufficient at demo scale?~~ **Resolved 2026-07-24, Karl Johnson:** anonymous access is sufficient — confirmed reachable and used as-is in Cornell's DOB resolver POC (`backend/apps/compliance/dob.py`) and `docs/architecture/geocoding-reachability-findings.md`, with no rate-limit issues at demo scale. No app token needed unless real usage patterns change post-MVP.
- ~~Is NYC Planning's GeoSearch API (assumed no-auth) actually reachable, or has it been retired in favor of the key-required Geoservice API?~~ **Resolved 2026-07-22, Cornell Robertson:** the HTTP 410 was a stale API version, not a dead service — GeoSearch **v2** (`https://geosearch.planninglabs.nyc/v2/search`) is reachable, keyless, and returns BIN directly; confirmed end-to-end (`120 Broadway` → BIN 1001026 → live DOB device data) in `docs/architecture/geocoding-reachability-findings.md` and used as-is in the working resolver POC (`backend/apps/compliance/dob.py`).
- ~~How should address→BIN resolution handle ambiguous matches (e.g., multiple buildings/BINs on one tax lot, or an address the resolver can't confidently match)? Owner: Cornell Robertson — needs a decision before the address-lookup flow ships (e.g., show a disambiguation list vs. take the first match).~~ **Resolved 2026-07-27, Karl Johnson:** disambiguate via a picker, never take the first match — confirmed with a real example (`"200 Water St"` resolves to 5 matches spanning Manhattan and Brooklyn) in `docs/architecture/geocoding-reachability-findings.md`. `POST /api/buildings/lookup/` now returns `reason: "ambiguous_match"` with a `matches` array of candidates when this occurs; the client renders those as a picker and re-calls the endpoint with the chosen candidate's `bin` to complete the lookup. See `docs/architecture/integration-contracts.md` §3.
