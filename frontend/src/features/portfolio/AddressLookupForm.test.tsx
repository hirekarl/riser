...
describe("AddressLookupForm", () => {
  ...
  it("shows the panel head (icon badge + eyebrow title/subtitle) above the address field", () => {
    render(<AddressLookupForm onSaved={vi.fn()} />);

    expect(screen.getByText("Building Lookup")).toBeInTheDocument();
    expect(screen.getByText("NYC DOB Integration")).toBeInTheDocument();
  });

  it("shows non-color-reliant helper text explaining the lookup never auto-saves", () => {
    render(<AddressLookupForm onSaved={vi.fn()} />);

    expect(screen.getByText(/nothing saves automatically/i)).toBeInTheDocument();
  });
